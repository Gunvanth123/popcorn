...
import { tmdbApi, popcornApi } from '../../api/client'
import { Film, Tv, Star, Plus, Check, Loader2, ArrowLeft, TrendingUp } from 'lucide-react'
...
export default function TrendingTop100() {
  ...
  const fetchTrendingAndWatchlist = async () => {
    ...
    try {
      ...
      toast.error('Failed to load trending titles')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 pb-16">
      ...
      <header className="border-b border-white/5 bg-slate-900/40 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-orange-500">
            ...
            <h1 className="font-black text-lg text-white">Trending Top 100</h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 mt-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h2 className="text-3xl font-black text-white">Weekly Top 100</h2>
            <p className="text-slate-400 text-sm mt-1">Discover popular movies, series, and anime trending this week.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {trending.map((item, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="group bg-slate-900/60 border border-slate-800/80 hover:border-orange-500/30 rounded-2xl overflow-hidden flex flex-col shadow-md hover:shadow-xl transition-all duration-300 relative"
                >
                  {/* Rank Badge */}
                  <div className="absolute top-3 left-3 w-8 h-8 rounded-lg bg-orange-600/90 text-white flex items-center justify-center font-black text-sm shadow-md z-10">
                    {idx + 1}
                  </div>
                  ...
                  <div className="aspect-[2/3] bg-slate-950 relative overflow-hidden shrink-0">
                    ...
                    {/* Popcorn Rating Tag */}
                    <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-slate-950/70 border border-white/10 text-[10px] font-black text-yellow-400 flex items-center gap-1 backdrop-blur-md">
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between min-h-0">
                    <div className="space-y-2">
                      <h3 className="font-bold text-sm text-white line-clamp-1 group-hover:text-orange-500 transition-colors" title={item.title}>
                        {item.title}
                      </h3>
